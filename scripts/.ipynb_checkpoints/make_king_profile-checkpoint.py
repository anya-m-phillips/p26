###### this is mostly pirated from COSMIC
# """
...
### this now only requires that I have numpy and scipy in this environment!
import numpy as np
...
#### below is exactly the COSMIC code. 
...
##### BELOW I HAVE MODIFIED TO TAKE A MASS ARRAY TO ENSURE PROPER SCALING TO NBODY UNITS

def scale_pos_and_vel(r,vr,vt, mass_array): ### assumes entries are radially sorted
    """
    ...
    mass = mass_array / np.sum(mass_array) 
    ...
    return r*rfac, vr*vfac, vt*vfac, mass # mass is scaled to n-body units

# def draw_r_vr_vt(mass_array, w_0=6, tidal_boundary=1e-6):
def draw_r_v(mass_array, w_0=6, tidal_boundary=1e-6):
    """
    Draw random velocities and positions from the King profile.

    mass_array     = numpy array of masses (i think in any units should be fine)
    w_0            = King concentration parameter (-psi/sigma^2) 
    tidal_boundary = ratio of rho/rho_0 where we truncate the tidal boundary

    returns (vr,vt,r,m) in G=M_cluster=1 units

    [ACTUALLY returns r, v, m where v is total velocity. orient isotropically somewhere else.]
    """
    N = len(mass_array)
    ...
    ### actually sort both r and mass_array
    # r = np.sort(r)
    sort_by_r = np.argsort(r)
    r = r[sort_by_r]
    mass_array = mass_array[sort_by_r]

    # Finally, draw the velocities using the radii and one of the Jeans
    # equations
    vr, vt = get_velocities(r, r_profile, psi_profile, M_enclosed_profile) # rs are sorted by radius here

    # Scale into Henon units
    r, vr, vt, m = scale_pos_and_vel(r,vr,vt, mass_array) # rs and mass_array are sorted by radius

    v = np.sqrt(vr**2 + vt**2)

    # replace r, v, m to input order of mass_array
    inverse_sort = np.argsort(sort_by_r)
    r = r[inverse_sort]
    v = v[inverse_sort]
    m = m[inverse_sort]
    # return r, vr, vt, m
